...
def _exact_int(value, name, *, minimum=None, maximum=None):
    """Accept numeric values only when they represent an exact finite integer."""
...
def _validate_sir(beta, gamma, I0, N, days, *, stochastic=False):
    ...
    I0 = _exact_int(I0, "I0", minimum=0)
    N = _exact_int(
        N,
        "N",
        minimum=1,
        maximum=MAX_GILLESPIE_POPULATION if stochastic else None,
    )
    if I0 > N:
...
def report_sir(args):
    ...
    match = (
        sim_final_size < max(0.05, 3 * I0 / N)
        if theory_final_size == 0
        else abs(sim_final_size - theory_final_size) < 5e-2
    )
    print(f"Theory match           = {match}")
    checks = {
        "population_conserved": bool(np.max(np.abs(S + I + R - N)) < max(1e-3, 1e-9 * N)),
        "compartments_nonnegative": bool(
            np.all(I >= -1e-6) and np.all(S >= -1e-6) and np.all(R >= -1e-6)
        ),
        "R_monotonic_increase": bool(np.all(np.diff(R) >= -1e-8)),
    }
    print("\n--- sanity checks ---")
    for key, value in checks.items():
        print(f"{key:35s} {'PASS' if value else 'FAIL'}")
    ...
    if args.plot:
        try:
            ...
            ax.annotate(
                f"peak {I[peak_idx]:,.0f}\nday {t[peak_idx]:.0f}",
                (t[peak_idx], I[peak_idx]),
                xytext=(8, -4),
...
def gillespie_sir_once(beta, gamma, I0, N, t_max=180.0, rng=None):
    beta, gamma, I0, N, t_max = _validate_sir(
        beta, gamma, I0, N, t_max, stochastic=True
    )
    rng = rng or np.random.default_rng()
    S, I = N - I0, I0
    t = 0.0
    peak_I = I0
    # In SIR each person can be infected at most once and recovered at most
    # once, so at most 2*N transitions are possible in one realization.
    events = 0
    event_limit = 2 * N
...
def _fadeout_probability(beta, gamma, I0):
    """Stable early-phase jump-chain fade-out probability."""
    if I0 == 0 or beta == 0:
        return 1.0
    r = gamma / beta
    ...
    if abs(r - 1.0) < 1e-9:
        return 1.0 / (I0 + 1.0)
    if r < 1.0:
        # Algebraically equivalent to (r-1)/(r-r**(-I0)), but avoids overflow.
        power = r ** I0
        denominator = 1.0 - r * power
        return (1.0 - r) * power / denominator if denominator > 0 else 0.0
    inverse_power = r ** (-I0)
    return (r - 1.0) / (r - inverse_power)


def report_gillespie(args):
    beta, gamma, I0, N, days = _validate_sir(
        args.beta, args.gamma, args.I0, args.N, args.days, stochastic=True
    )
    runs = _exact_int(args.runs, "runs", minimum=1, maximum=MAX_RUNS)
    sweep_i0 = [v for v in [1, 5, 10, 25, 50] if v <= N] if args.sweep else []
    total_requested_runs = runs + 200 * len(sweep_i0)
    if total_requested_runs * N > MAX_GILLESPIE_WORK:
        raise ValueError(
            f"all requested Gillespie runs*N exceed hard stochastic work limit {MAX_GILLESPIE_WORK}"
        )

    rng = np.random.default_rng(args.seed)
    results = [
        gillespie_sir_once(beta, gamma, I0, N, t_max=days, rng=rng)
        for _ in range(runs)
    ]
    ...
    p_fadeout_theory = _fadeout_probability(beta, gamma, I0)
    ...
    print(
        f"final size 5-95% interval   = "
        f"[{np.percentile(finals, 5):.4f}, {np.percentile(finals, 95):.4f}]"
    )
    major_mask = ~np.array([r["extinct_early"] for r in results])
    major = finals[major_mask]
    if len(major) > 0:
        big = int(np.sum(major > 0.1))
        print(
            f"non-fade-out runs           = {len(major)}, "
            f"of which true outbreaks (>10% infected): {big}"
        )
        if big:
            print(
                f"true-outbreak final size    = {major[major > 0.1].mean():.4f} "
                f"(deterministic: {det_final:.4f})"
            )
    tol = max(
        0.04,
        3 * math.sqrt(max(p_fadeout_theory * (1 - p_fadeout_theory), 1e-6) / runs),
    )
    checks = {
...
def erlang_c(lam, mu, c):
    lam = _finite(lam, "lambda")
    mu = _finite(mu, "mu")
    c = _exact_int(c, "staff", minimum=1, maximum=MAX_QUEUE_STAFF)
    if lam < 0 or mu <= 0:
        raise ValueError("queue requires lambda >= 0 and mu > 0")
    if lam == 0:
        return 0.0, 0.0
    a = lam / mu
    rho = a / c
    ...
    return p_wait, p_wait / (c * mu - lam)


def report_queue(args):
    lam = _finite(args.lam, "lambda")
    mu = _finite(args.mu, "mu")
    target_min = _finite(args.target_wait, "target_wait")
    if lam < 0 or mu <= 0 or target_min < 0:
        raise ValueError("queue requires lambda >= 0, mu > 0, target_wait >= 0")
    c_min = max(1, math.ceil(lam / mu))
    if c_min > MAX_QUEUE_STAFF:
        raise ValueError(f"required baseline staff exceeds hard limit {MAX_QUEUE_STAFF}")
    ...
    while feasible is None and c <= min(c_min + 100, MAX_QUEUE_STAFF):
        ...
        feasible = c if meets else feasible
        c += 1
    if not rows:
        return 1
    ...
    print(
        f"at c*-1 the average wait was {prev_w:.2f} min vs "
        f"{rows[first_feasible_idx][3]:.2f} min -- the staffing cliff"
    )
    checks = {
        "c*_above_fluid_bound": feasible >= c_min,
        "wait_decreases_with_staff": all(
            rows[i][3] >= rows[i + 1][3] for i in range(len(rows) - 1)
        ),
        "target_met_at_c*": erlang_c(lam, mu, feasible)[1] * 60 <= target_min + 1e-9,
    }
    print("\n--- sanity checks ---")
    for key, value in checks.items():
        print(f"{key:35s} {'PASS' if value else 'FAIL'}")
    if args.sweep:
        ...
        for m in range(max(5, int(mu) - 5), int(mu) + 6, 2):
            baseline = max(1, math.ceil(lam / m))
            cc = baseline
            while cc <= MAX_QUEUE_STAFF:
                _, wh = erlang_c(lam, m, cc)
                if wh * 60 <= target_min or cc > baseline + 100:
                    break
                cc += 1
            print(f"{m:5d} {cc:4d}")
    return 0 if all(checks.values()) else 1
...
def main():
    ...
    p.add_argument("--I0", type=int, default=10)
    p.add_argument("--N", type=int, default=1_000_000)
    p.add_argument("--runs", type=int, default=300)
    p.add_argument("--seed", type=int, default=42)
    p.add_argument("--lam", type=float, default=60)
    p.add_argument("--mu", type=float, default=20)
    p.add_argument("--target-wait", type=float, default=3)
    p.add_argument("--days", type=int, default=180)
    p.add_argument("--sweep", action="store_true")
    p.add_argument("--plot", metavar="PNG", default=None)
    args = p.parse_args()
    try:
        code = MODELS[args.model](args)
