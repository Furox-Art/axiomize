...
1. Re-scan the archetype catalog asking what this MOST resembles, not what matches.
2. Declare the analogy precisely: e.g. 'Structure resembles SIR in WHO moves between states, but transfer rates are chosen, not contact-driven.'
3. Split explicitly: which parts of the borrowed math TRANSFER (keep), which FAIL (discard), which are UNKNOWN (mark `[S]`).
...
- proportional to source only → linear decay/growth
- proportional to source × target → mass-action mixing
- saturating with availability → Michaelis-Menten / Hill forms
- triggered by thresholds → piecewise / regime-switching
- delayed by transport → delay terms (note stability risk)
...
3. What pattern in that data would force immediate restructure?
...
- research tier: minimum THREE lenses built fully, from unrelated mathematical families
- if two lenses agree on the key prediction → report that convergence explicitly
- if they disagree → that disagreement IS the headline result; do not bury it

Report additions (novel-territory appendix):

| Field | Content |
|-------|---------|
| Nearest analogy | declared in Step 1, with keep/fail/unknown split |
| Conserved stocks | from Step 2 |
| Dimensionless groups | from Step 4 |
| Minimum data plan | from Step 6 |
| Confidence ledger | everything defaults to `[S]` until measured |

## Anti-patterns

- Analogy denial: claiming nothing resembles anything (there is always a nearest neighbor)
- Equation smuggling: copying an archetype formula while claiming first-principles
- Complexity theater: five stock variables where one answers the question
- Prediction without commitment: falsifiers phrased so vaguely nothing could fail
